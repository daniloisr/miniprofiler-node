...
for (let framework of ['koa', 'express', 'hapi']) {
  ...
  exports[framework] = function(options = {}) {
    ...
    return func.mainMiddleware(options, handleRequest);
  };
}
...
function handleRequest(enable, authorize, req, res) {
  return new Promise((resolve, reject) => {
    ...
    if (!requestPath.startsWith(resourcePath)) {
      ...
      if (enabled) {
        res.on('finish', () => {
          try { stopProfiling(req); }
          catch (err) {
            // Exception can happen on stopProfiling when the start-end profile step is
            // interrupted by an client-side exception.
            // This catch block don't need to do anything because we don't need to
            // profile a session with errors.
          }
        });
