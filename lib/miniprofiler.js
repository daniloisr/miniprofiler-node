...
function handleRequest(enable, authorize, req, res) {
  return new Promise((resolve, reject) => {
    ...
    if (!requestPath.startsWith(resourcePath)) {
      ...
      if (enabled) {
        res.on('finish', stopProfiling.bind(null, req));
...
function describeCustomTimings(customTimings, root) {
  ...
  for(var prop in customTimings) {
    ...
    for(var i = 0; i < arr.length; i++) {
      // stopTime can be undefined when the provider throw an exception between "startTimeQuery" and "stopTimeQuery"
      if (!arr[i].stopTime) continue;
